...
/*   Updated: 2019/07/31 15:27:55 by mqian            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     format_s_right(int flags[], char *temp, int len)
{
	...
	else if (flags[5] >= len && flags[5] >= flags[7])
	{
		if (len >= flags[7] && flags[7] > 0)
			...
			count += format_s_right_helper_2(flags, temp, len); //precision ignored since greater than len
		
	}
	else if (flags[7] >= len && flags[7] >= flags[5]) //precision ignored
        count += format_s_right_helper_3(flags, temp, len);
    return (count);
}
...
{
    //need to handle case where precision 
