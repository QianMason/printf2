...
/*   Updated: 2019/08/19 15:10:11 by mqian            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		format_o_left(int flags[], uintmax_t argument, int len)
{
    ...
    {
        if (len >= flags[7])
            count += format_o_left_helper_2(flags, argument);
        else
...
int		format_o_left_helper_1(int flags[], uintmax_t arg, int len)
{
    ...
    count += (arg > 0) ? convert_to_octal(arg, 1) : write_and_increment('0');
    return (count);
}
...
int		format_o_left_helper_2(int flags[], uintmax_t arg)
{
    ...
    if (flags[7] == 0 && flags[6] == 1)
        count += (arg > 0) ? convert_to_octal(arg, 1) : write_and_increment(' ');
    else
        count += (arg > 0) ? convert_to_octal(arg, 1) : write_and_increment('0');
    while (count < flags[5])
